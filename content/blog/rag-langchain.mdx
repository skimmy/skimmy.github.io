...
Pre-trained Large Language Models (LLM), like ChatGPT, do a tremendous job at generating answers to our questions, re-organizing the information they acquired during training (the way they store information is nothing like a Wikipedia page). Often, we want models to use only part of such information or to use information they didn’t have access at training time (for example the internal documents of our organization). 
...
In this post, we focus on the second solution. This is the first of a series of posts on RAG. While here we cover the whole pipeline, subsequent posts will focus on specific parts.
...
**RAG uses previously indexed documents to build a context based on the query to which it is added to construct the final prompt for the LLM**.

## Preliminaries

I'll assume the you are familiar with Python and basic package management. Besides that, there are two things that you need to do before preparing for this guide
...
The first step in creating a RAG system is to loadn and index the documents of interest. I demonstrate the potential of RAG using this [cat facts](https://huggingface.co/ngxson/demo_simple_rag_py/blob/main/cat-facts.txt) dataset that has already been cleaned and converted to plain text.



> [!WARNING]
> The quality of indexed documents is crucial for the performance of your RAG system. The way in which content is organized heavily impacts on the LLM responses.


### Loading and splitting 📂

First, we load all files to index, LangChain has a broad selection of [Document Loaders](https://python.langchain.com/docs/integrations/document_loaders/) (see also the [API Documentation](https://python.langchain.com/api_reference/community/document_loaders.html)), you won’t be short on options!
...
After loading, documents are *split* into *chunks*. Splitting helps overcoming some issues: unequal documents length, limited LLM context, … When documents are to be used with an LLM, it is a good idea to perform a *tokenization-based* split. To keep things simple, we split on newline separator `\n`, this is coherent with the content of our file. More on LangChain splitters can be found inthe [documentation](https://python.langchain.com/docs/concepts/text_splitters/).
...
> [!WARNING] 
> This step may generate several warnings that splits exceed the 100 characters limits. I decided to ignore them, but it could be better to use higher `chunk_size`.


### Storing vectors 🗄️

Next comes indexing, where documents are transformed in *embedding vectors* to store in a *vector store* (so that they can be later retrieved). For such transformation, we need an *embedding model*, I uses `OllamaEmbeddings` since I’m using Ollama chat model, however, **it is not required to use an embedding that matches the chat model** since we will translate them back to text to create the prompt.
...
It is worth mentioning the model `llama3.2:1b` is running locally on my M1 laptop using Ollama. We are now ready to convert documents into vectors and put them in the store. In this example, I used `InMemoryVectorStore` from LangChain, but you can based on your needs, visit the  [LangChain Vector stores documentation](https://python.langchain.com/docs/integrations/vectorstores/).
...
Having stored our documents, we can use them to retrieve relevant information for any given query, we do that by running a *similarity search* on our store.
...
In one case I got the following answer containing a useful first part and a less useful second part.
...
we can set up a first RAG application with under 40 lines of code
...
In this post I presented a simple RAG application that uses a text file and a locally running LLM perform an augmented text generation. This is an invaluable tool to start tinkering with some documents and Large Language Models without going through all the costs and issues of an entire fine tuning. I barely scratched the surface of the LangChain library and subsequent posts will focus on ways to make the above code more flexible, stay tuned 📻.
